...
// NUEVA FUNCIÓN PARA FORMATEAR EL PRECIO EN EL INPUT EN TIEMPO REAL
function formatPrecioInput(event) {
    let value = event.target.value;
    // 1. Eliminar cualquier caracter que no sea un dígito
    let numeroLimpio = value.replace(/\D/g, '');

    // 2. Si está vacío después de limpiar, simplemente retornar (o poner '0' si se prefiere)
    if (!numeroLimpio) {
        event.target.value = '';
        return;
    }

    // 3. Convertir a número y luego a string con formato local para Argentina
    // Esto añadirá los puntos como separadores de miles.
    const numero = parseInt(numeroLimpio, 10);
    event.target.value = numero.toLocaleString('es-AR');
}

// NUEVA FUNCIÓN PARA OBTENER EL VALOR NUMÉRICO REAL DEL PRECIO FORMATEADO
function getPrecioNumerico(formattedValue) {
    if (!formattedValue) return 0;
    // Eliminar los puntos (separadores de miles) antes de convertir a número
    const numeroLimpio = formattedValue.replace(/\./g, '');
    return parseInt(numeroLimpio, 10) || 0; // Devolver 0 si no es un número válido
}
...
function resetFormularioProducto() {
    ...
    if(precioProductoInputEl) precioProductoInputEl.value = ''; // Limpiar campo de precio
}
...
async function handleProductoSubmit(event) {
    ...
    // OBTENER VALOR NUMÉRICO REAL DEL PRECIO
    const precio = getPrecioNumerico(precioProductoInputEl.value); 
...
function actualizarListaProductosHTML(productos, esActualizacionDesdeFirestore) {
    ...
    productosVisibles.forEach(producto => {
        ...
        const precioFormateado = precioValido 
            ? `$ ${parseInt(producto.precio, 10).toLocaleString('es-AR')}` 
            : 'N/A';
...
async function cargarProductoParaEdicion(id) {
    ...
    try {
        ...
        if (producto) {
            ...
            // CARGAR PRECIO FORMATEADO EN EL INPUT AL EDITAR
            if(precioProductoInputEl && producto.precio !== undefined) {
                const precioNumerico = parseInt(producto.precio, 10);
                precioProductoInputEl.value = isNaN(precioNumerico) ? '' : precioNumerico.toLocaleString('es-AR');
...
export function initProductos(elements) {
    ...
    // AÑADIR LISTENER PARA FORMATEAR PRECIO EN TIEMPO REAL
    if (precioProductoInputEl) {
        // Cambiamos a 'input' para que se actualice mientras se escribe
        // y 'blur' para formatear si pegan un valor y salen del campo.
        precioProductoInputEl.addEventListener('input', formatPrecioInput);
        precioProductoInputEl.addEventListener('blur', (event) => {
            // Al salir del campo, si hay un valor, lo re-formateamos por si acaso
            // y para asegurar que se aplique el formato si se pegó un valor.
            let value = event.target.value;
            let numeroLimpio = value.replace(/\D/g, '');
            if (numeroLimpio) {
                const numero = parseInt(numeroLimpio, 10);
                ...
                event.target.value = ''; // Limpiar si no es un número válido
            }
        });
    }
}